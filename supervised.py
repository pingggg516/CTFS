...
def evaluate(model, loader, mode, cfg, multiplier=None):
    ...
    with torch.no_grad():
        for img, mask, id in loader:
            ...
            mask = mask.cuda(non_blocking=True) if hasattr(mask, 'cuda') else mask
            ...
            if mode == 'sliding_window':
                ...
                for i in range(img.shape[0]):
                    ...
                    while row < h:
                        col = 0
                        while col < w:
                            pred = model(single_img[:, :, row: row + grid, col: col + grid])
                ...
                for pred_np, mask_np in zip(batch_preds, batch_masks):
                    intersection, union, target =\
                        intersectionAndUnion(pred_np, mask_np, cfg['nclass'], 255)
                ...
                for i in range(pred_np.shape[0]):
                    intersection, union, target =\
                        intersectionAndUnion(pred_np[i], mask_np[i], cfg['nclass'], 255)
...
def main():
    ...
    trainloader = DataLoader(
        trainset, batch_size=cfg['batch_size'], pin_memory=True, num_workers=4, drop_last=True, shuffle=True
    )


    valloader = DataLoader(
        valset, batch_size=4, pin_memory=True, num_workers=4, drop_last=False, shuffle=False
    )
